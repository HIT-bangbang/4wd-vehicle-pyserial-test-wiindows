...
#在本代码中测试带有time.sleep的串口读取（即每读到一次串口完成的数据包后休眠一段时间再读取）
#目的是为了解决ros中读取时，发布消息时存在rate.sleep，导致串口堵塞，读取的数据存在巨大的延迟
#目前的解决办法是在每次读取到一个完整的数据包之后，关闭串口，（serial.close()函数）
#下次读取时再打开串口
            ...
            if (rx_con < (rx_buf[2]-1)):
                ...
                if(res == (rx_checksum & 0xff)): #校验正确
                    ...
                    if(rx_buf[3] == 6):
                        ...
                        ser.close()
                        time.sleep(0.1)  #在这里引入了延时.如果没有反复关开串口的操作会导致读取到的数据巨大的延迟
                        pass
                        pass
                    rx_con = 0
